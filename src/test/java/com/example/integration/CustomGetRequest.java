...
import com.couchbase.client.core.retry.RetryStrategy;
  ...
  * @since 2.0.0
  */
 public class CustomGetRequest extends BaseKeyValueRequest<CustomGetResponse> {
 
   private final String key; 
    
   public CustomGetRequest(final String key, final Duration timeout, final CoreContext ctx,
   ...
   @Override
   public CustomGetResponse decode(final ByteBuf response, KeyValueChannelContext ctx) {
     ...
     if (status.success()) {
       ...
       return new CustomGetResponse(key, status, content, cas, flags);
     } else {
       return new CustomGetResponse(key, status, null, cas, 0);
     }
   }
